############################################################################################
	# Mining Reset Stone Experience #
############################################################################################
scoreboard players reset @s[scores={mined_stone=1..}] mined_stone
scoreboard players reset @s[scores={mined_andesite=1..}] mined_andesite
scoreboard players reset @s[scores={mined_granite=1..}] mined_granite
scoreboard players reset @s[scores={mined_diorite=1..}] mined_diorite
scoreboard players reset @s[scores={mined_obsidian=1..}] mined_obsidian


############################################################################################
	# Mining Reset Stone Experience #
############################################################################################
scoreboard players reset @s[scores={mined_dripstone=1..}] mined_dripstone
scoreboard players reset @s[scores={mined_deepslate=1..}] mined_deepslate
scoreboard players reset @s[scores={mined_calcite=1..}] mined_calcite
scoreboard players reset @s[scores={mined_tuff=1..}] mined_tuff
scoreboard players reset @s[scores={mined_smo_basalt=1..}] mined_smo_basalt
scoreboard players reset @s[scores={mined_amethyst=1..}] mined_amethyst


############################################################################################
	# Mining Reset Ore Experience #
############################################################################################
scoreboard players reset @s[scores={mined_coal=1..}] mined_coal
scoreboard players reset @s[scores={mined_iron=1..}] mined_iron
scoreboard players reset @s[scores={mined_gold=1..}] mined_gold
scoreboard players reset @s[scores={mined_copper=1..}] mined_copper
scoreboard players reset @s[scores={mined_lapis=1..}] mined_lapis
scoreboard players reset @s[scores={mined_emerald=1..}] mined_emerald
scoreboard players reset @s[scores={mined_redstone=1..}] mined_redstone
scoreboard players reset @s[scores={mined_diamond=1..}] mined_diamond


############################################################################################
	# Mining Deepslate Reset Ore Experience #
############################################################################################
scoreboard players reset @s[scores={mined_deep_coal=1..}] mined_deep_coal
scoreboard players reset @s[scores={mined_deep_iron=1..}] mined_deep_iron
scoreboard players reset @s[scores={mined_deep_gold=1..}] mined_deep_gold
scoreboard players reset @s[scores={mined_deep_cop=1..}] mined_deep_cop
scoreboard players reset @s[scores={mined_deep_lapis=1..}] mined_deep_lapis
scoreboard players reset @s[scores={mined_deep_emer=1..}] mined_deep_emer
scoreboard players reset @s[scores={mined_deep_redst=1..}] mined_deep_redst
scoreboard players reset @s[scores={mined_deep_diam=1..}] mined_deep_diam


############################################################################################
	# Mining Reset Terracotta Experience #
############################################################################################
scoreboard players reset @s[scores={mined_terracotta=1..}] mined_terracotta
scoreboard players reset @s[scores={mined_white_terr=1..}] mined_white_terr
scoreboard players reset @s[scores={mined_orang_terr=1..}] mined_orang_terr
scoreboard players reset @s[scores={mined_magen_terr=1..}] mined_magen_terr
scoreboard players reset @s[scores={mined_ltblu_terr=1..}] mined_ltblu_terr
scoreboard players reset @s[scores={mined_yello_terr=1..}] mined_yello_terr
scoreboard players reset @s[scores={mined_lime_terr=1..}] mined_lime_terr
scoreboard players reset @s[scores={mined_pink_terr=1..}] mined_pink_terr
scoreboard players reset @s[scores={mined_gray_terr=1..}] mined_gray_terr
scoreboard players reset @s[scores={mined_ltgra_terr=1..}] mined_ltgra_terr
scoreboard players reset @s[scores={mined_cyan_terr=1..}] mined_cyan_terr
scoreboard players reset @s[scores={mined_purpl_terr=1..}] mined_purpl_terr
scoreboard players reset @s[scores={mined_blue_terr=1..}] mined_blue_terr
scoreboard players reset @s[scores={mined_brown_terr=1..}] mined_brown_terr
scoreboard players reset @s[scores={mined_green_terr=1..}] mined_green_terr
scoreboard players reset @s[scores={mined_red_terr=1..}] mined_red_terr
scoreboard players reset @s[scores={mined_black_terr=1..}] mined_black_terr


############################################################################################
	# Mining Reset Sandstone Experience #
############################################################################################
scoreboard players reset @s[scores={mined_sandstone=1..}] mined_sandstone
scoreboard players reset @s[scores={mined_red_sandst=1..}] mined_red_sandst


############################################################################################
	# Mining Reset "The Nether" Experience #
############################################################################################
scoreboard players reset @s[scores={mined_netherrack=1..}] mined_netherrack
scoreboard players reset @s[scores={mined_neth_gold=1..}] mined_neth_gold
scoreboard players reset @s[scores={mined_nether_qua=1..}] mined_nether_qua
scoreboard players reset @s[scores={mined_netherbric=1..}] mined_netherbric
scoreboard players reset @s[scores={mined_magma_bloc=1..}] mined_magma_bloc

############################################################################################
	# Mining Reset "The Nether" NEW BIOMES: Experience #
############################################################################################
scoreboard players reset @s[scores={mined_crim_nyli=1..}] mined_crim_nyli
scoreboard players reset @s[scores={mined_warp_nyli=1..}] mined_warp_nyli
scoreboard players reset @s[scores={mined_neth_gold=1..}] mined_neth_gold
scoreboard players reset @s[scores={mined_blackstone=1..}] mined_blackstone
scoreboard players reset @s[scores={mined_gilded=1..}] mined_gilded
scoreboard players reset @s[scores={mined_anc_debris=1..}] mined_anc_debris
